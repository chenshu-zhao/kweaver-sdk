...
当 `~/.kweaver/` 没有选中平台时，若已设置 **`KWEAVER_BASE_URL`**，`config show` 可用该 URL 作为平台（输出会标注 `(KWEAVER_BASE_URL)`）。`config list-bd` 同样支持 env 兜底，需要同时提供 **`KWEAVER_TOKEN`**（或任何可被 `ensureValidToken` 解析的凭证）以完成后端查询。`config set-bd` 仅写磁盘，**要求已登录并选中平台**。
